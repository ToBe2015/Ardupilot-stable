...
#define AP_MOTORS_SPIN_UP_TIME          0.5f    // time to go from zero throttle to spin when armed, spin when armed to min throttle, and min throttle to full throttle.
...
public:
    ...
    // set_yaw_headroom - set yaw headroom (yaw is given at least this amount of thrust) todo: Change this to a factor 0 to 1
    void                set_yaw_headroom(int16_t headroom_factor) { _yaw_headroom = headroom_factor/1000.0f; }
    ...
    void                set_throttle_mix_zero() { _throttle_rpy_mix_desired = 0.0f; _throttle_rpy_mix = 0.0f;}
    ...
    //  todo
    void                set_spool_desired_shut_down() { _multicopter_flags.spool_desired = DESIRED_SHUT_DOWN;}
    void                set_spool_desired_spin_when_armed() { _multicopter_flags.spool_desired = DESIRED_SPIN_WHEN_ARMED;}
    void                set_spool_desired_min_throttle() { _multicopter_flags.spool_desired = DESIRED_SPIN_MIN_THROTTLE;}
    void                set_spool_desired_full_throttle() { _multicopter_flags.spool_desired = DESIRED_FULL_THROTTLE;}
    ...
    int16_t             throttle_max() const { return _max_motor_out; }
    int16_t             throttle_min() const { return rel_pwm_to_thr_range(_min_motor_out); }
    ...
    void                set_hover_throttle(float hov_thr) { _throttle_thrust_hover = hov_thr/1000.0f; }
...
protected:
    ...
    // update_max_throttle - updates the limits on _max_motor_out for slow_start and current limiting flag
    ...
    // convert RPY and Throttle servo ranges from legacy controller scheme back into normalized thrust values
    // RPY channels typically +/-45 degrees servo travel between +/-1.0 thrust
    // Throttle channel typically 0-1000 range converts to 0.0-1.0 thrust for final output signal to motors
    ...
    enum spool_up_down_mode {
        ...
        SPIN_WHEN_ARMED_SPOOL_UP = 1,      // ramp throttle to spin when armed
        SPIN_WHEN_ARMED = 2,    // all motors at spin when armed
        SPIN_MIN_THROTTLE_SPOOL_UP = 3,      // ramp throttle to spin when armed
        SPIN_MIN_THROTTLE = 4,    // all motors at spin when armed
        ...
        SPIN_MIN_THROTTLE_SPOOL_DOWN = 8,      // ramp throttle to spin when armed
        SPIN_WHEN_ARMED_SPOOL_DOWN = 9,      // ramp throttle to spin when armed
    };
    ...
    enum spool_up_down_desired {
        ...
        DESIRED_SPIN_MIN_THROTTLE = 2,    // all motors at spin when armed
        DESIRED_FULL_THROTTLE = 3,      // throttle is no longer constrained by start up procedure
    };

    // flag bitmask
    struct {
        spool_up_down_mode  spool_mode          : 4;    // see AutoTuneTuneMode for what modes are allowed
        spool_up_down_desired  spool_desired          : 2;    // see AutoTuneTuneMode for what modes are allowed
    ...
    AP_Float            _thrust_curve_expo;     // curve used to linearize thrust conversion.  range of 0.0 to 1.0. 0.0 for linear and 1.0 for second order approximation
    AP_Float            _thrust_curve_max;      // throttle which produces the maximum thrust.  (i.e. 0.0 ~ 1.0 ) of the full throttle range
    ...
    // internal variables - motor mixing
    ...
    float               _throttle_thrust_hover;     // the estimated hover throttle thrust 0.0 to 1.0
    float               _throttle_thrust_max;       // the maximum allowed throttle thrust 0.0 to 1.0
    float               _thrust_rpyt_out[AP_MOTORS_MAX_NUM_MOTORS];   // final thrust before conversion to pwm output

    // internal variables - ramp up
    int16_t             _spin_when_armed_ramped;    // equal to _spin_when_armed parameter but slowly ramped up from zero
    int16_t             _slow_start;                // slow start limit
    int16_t             _slow_start_spin_when_armed;                // slow start limit
    int16_t             _slow_start_min_throttle;                // slow start limit
