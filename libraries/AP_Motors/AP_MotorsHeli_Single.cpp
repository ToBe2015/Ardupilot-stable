...
void AP_MotorsHeli_Single::update_motor_control(AP_MotorsHeli_RSC::MotorControlState state)
{
    ...
    if (state == AP_MotorsHeli_RSC::ROTOR_CONTROL_STOP){
        // set motor run enable aux output to not run position to kill engine when disarmed
        RC_Channel_aux::set_radio_to_min(RC_Channel_aux::k_motor_run_enable);
    } else {
        // else if armed, set motor run enable output to mrun position
        // set servo PWM to deployed position
        RC_Channel_aux::set_radio_to_max(RC_Channel_aux::k_motor_run_enable);
    }
